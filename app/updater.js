...
platform = /^win/.test(platform)
  ? "win"
  : /^darwin/.test(platform)
  ? "mac"
...
var getZipDestinationDirectory = function (zipPath, temporaryDirectory) {
  ...
  getExecPathRelativeToPackage = function (manifest) {
    var execPath =
      manifest.packages[platform] && manifest.packages[platform].execPath;
...
var pUnpack = {
  ...
  mac: function (filename, cb, manifest, temporaryDirectory) {
    ...
    if (extension === ".zip") {
      ...
      exec(
        "hdiutil unmount /Volumes/" + path.basename(filename, ".dmg"),
        function (err) {
          ...
          exec(
            ...
            function (err) {
              exec(
                'hdiutil attach "' + cdrPath + '" -nobrowse',
                function (err) {
                  if (err) {
                    if (err.code == 1) {
  ...
  win: function (filename, cb, manifest, temporaryDirectory) {
    var destinationDirectory = getZipDestinationDirectory(
        filename,
        temporaryDirectory
      ),
      unzip = function () {
        ...
        exec(
          '"' +
            path.resolve(__dirname, "tools/unzip.exe") +
            '" -u -o "' +
            filename +
            '" -d "' +
  ...
  linux32: function (filename, cb, manifest, temporaryDirectory) {
    ...
    exec(
      ...
      function (err) {
        ...
        cb(
          null,
          path.join(temporaryDirectory, getExecPathRelativeToPackage(manifest))
        );
      }
    );
  },
...
var pRun = {
  ...
  linux32: function (appPath, args, options, cb) {
    ...
    options.cwd = appPath;
    return run(
      appPath + "/" + path.basename(this.getAppExec()),
      args,
      options,
      cb
    );
...
var pInstall = {
   ...
   */
  mac: function (to, cb) {
    cp(this.getAppPath(), to).catch(cb);
  },
  /**
   * @private
   */
  win: function (to, cb) {
    ...
    function appDeleted(err) {
      if (err) {
        ...
        cp(self.getAppPath(), to).catch(appCopied);
      }
    }
